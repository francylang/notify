...
const Classroom = ({ teacher, students, deleteStudent, userStatus, coreValues, resetCount, count }) => {

  const sendSummary = (students, coreValues) => {
    // returnIfValueHasCount();
        ...
        <div className='student-classroom'>
        {
          students.map(({ name, id }) => {
            return <StudentLink
              ...
              coreValue={ Object.values(coreValues).map((coreValue, index) => {
                return Object.keys(coreValue)
              })
              }
            />;
          })
