...
describe('<Header />', () => {
  ...
  it('should render a link to create a new student', () => {
    const wrapper = shallow(<Header />);

    const newStudentLink = wrapper.find('.new-student-link');

    expect(newStudentLink.length).toEqual(1);
  });

  it('should render a link to create a new core value', () => {
    const wrapper = shallow(<Header />);

    const newCoreValueLink = wrapper.find('.new-value-link');

    expect(newCoreValueLink.length).toEqual(1);
  });
