...
const characterReducer = (state = coreValueData, action) => {
  ...
  case 'RETURN_IF_VALUE_HAS_COUNT':
    const moreThanZero = currentValues[action.coreValue].length > 0;
    const senObj = Object.assign({}, { [action.coreValue]: moreThanZero });
    return Object.assign({}, {[action.studentId]: senObj});
  default:
    return state;
  }
};
